...
async def test_confirm_vs_cancel_race(hotel_service_single):
    ...
    try:
        ...
        # Cancellation is valid from pending OR confirmed, so both may succeed
        # if confirm runs first. Assert cancellation always wins final state.
        persisted = await database.db.bookings.find_one({"booking_id": booking["booking_id"]})
